...
class DayView(arcade.View):
    ...
    def setup(self):
        ...
        arcade.set_background_color(self.tile_map.background_color or arcade.color.SKY_BLUE)
        if self.sound_manager:
            self.sound_manager.play_music("pixel_music.wav")
    ...
    def on_update(self, delta_time):
        ...
        if self.fade_state == "FADE_IN":
            ...
            # --- [QUAN TRỌNG] FIX LỖI TRÔI KHI BỊ ĐÁNH ---
            # Chỉ cho phép điều khiển di chuyển khi KHÔNG BỊ ĐAU
            if not self.soldier.is_hurting:
                self.soldier.change_x = 0
                current_speed = PLAYER_RUN_SPEED if self.shift_pressed else PLAYER_WALK_SPEED
                self.soldier.is_running = self.shift_pressed

                if self.left_pressed and not self.right_pressed:
                    self.soldier.change_x = -current_speed
                elif self.right_pressed and not self.left_pressed:
                    self.soldier.change_x = current_speed
            # Nếu đang bị đau (is_hurting = True), ta KHÔNG can thiệp vào change_x
            # để lực đẩy lùi (knockback) từ hàm take_damage hoạt động tự nhiên.
            # ---------------------------------------------
            ...
            if self.orc_bot in self.scene["OrcBot"]:
                if self.orc_bot.left < 0: self.orc_bot.left, self.orc_bot.change_x = 0, 0
                if self.orc_bot.right > self.map_width_pixels:
                    self.orc_bot.right, self.orc_bot.change_x = self.map_width_pixels, 0
    ...
    def switch_to_next_level(self):
        next_view = HallwayDayNightView(self.sound_manager)
        next_view.setup()
        self.window.show_view(next_view)

    def on_key_press(self, key, modifiers):
        if self.fade_state != "PLAYING": return
        # Nếu đang bị đau thì không nhận lệnh nhảy
        ...
        if key in (arcade.key.LSHIFT, arcade.key.RSHIFT):
            ...
            self.soldier.attack()
            # Gọi key "sword" chuẩn xác
            if self.sound_manager: self.sound_manager.play_effect("sword")

    def on_key_release(self, key, modifiers):
        # Lưu ý: Vẫn cho phép nhả phím (update cờ) kể cả khi đang bị đau
        # Để tránh trường hợp bị đau xong nhân vật tự chạy tiếp
